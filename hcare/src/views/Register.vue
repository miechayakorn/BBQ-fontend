...
export default {
  ...
  methods: {
    checkForm(){
        ...
        let emailSub = this.dataUserInfo.email.slice(email.length - 17, email.length)
        ...
        if(this.dataUserInfo.password == ""){
            ...
            return false
        }else if(emailSub != emailKmutt){
            return false
        }   
        return true
    },
    sendToBackend() {
      if (this.checkForm() && this.dataUserInfo.password == this.dataUserInfo.confirmpassword) {
        this.$swal({
          ...waiting,
          onOpen: () => {
